...
const WaterRing = forwardRef<WaterRingRef, WaterRingProps>(function WaterRing(
  ...
  const bubblesMultiplier =
    perf === 'high' ? 1.2 : perf === 'balanced' ? 1 : 0.7;
  const scaledIdleAmplitude =
    (shouldReduceMotion ? idleAmplitude * 0.5 : idleAmplitude) *
    scaleFactor *
    perfMultiplier;
  const scaledSplashAmplitudeMax =
    (shouldReduceMotion ? splashAmplitudeMax * 0.5 : splashAmplitudeMax) *
    scaleFactor *
    perfMultiplier;
  const scaledIdlePeriodMs = shouldReduceMotion
    ? idlePeriodMs * 2
    : idlePeriodMs;
  const scaledLevelRiseDurationMs = shouldReduceMotion
    ? levelRiseDurationMs * 2
    : levelRiseDurationMs;
  const scaledSplashDurationMs = shouldReduceMotion
    ? splashDurationMs * 2
    : splashDurationMs;
  ...
  // Goal achieved ring animation (0 = normal rim, 1 = green gradient)
  const goalAchievedOpacity = useSharedValue(progress >= 1 ? 1 : 0);
  ...
  const sampleCount = Math.min(
    32,
    Math.max(18, Math.round((22 + scaleFactor * 4) * perfMultiplier)),
  );
  ...
  useImperativeHandle(ref, () => ({
    ...
    triggerSplash: (deltaFraction?: number) => {
      ...
      const mappedLinear =
        6 + ((scaledSplashAmplitudeMax - 6) * Math.min(d, 0.2)) / 0.2;
      ...
      const combined = clamp(
        mapped + lastDeltaNorm.value * remaining * scaledSplashAmplitudeMax,
        0,
      ...
      splashT.value = 0;
      splashT.value = withTiming(1, {
        duration: scaledSplashDurationMs,
        easing: Easing.linear,
      });
    },
  }));
  ...
  useEffect(() => {
    ...
    // Animate goal achieved ring transition
    const isGoalAchieved = clampedNext >= 1;
    goalAchievedOpacity.value = withTiming(isGoalAchieved ? 1 : 0, {
      duration: 700,
      easing: Easing.inOut(Easing.ease),
    });
  }, [
    progress,
    scaledLevelRiseDurationMs,
  ...
  const frontWavePath = useDerivedValue(() => {
    ...
    const A_idle =
      scaledIdleAmplitude * (shouldReduceMotion ? 1 : 1 + 0.2 * breathPhase);
    ...
    const A_impulse =
      A_peak.value * Math.exp(-t / tau) * Math.cos(2 * Math.PI * f_splash * t);
    ...
    const slopeImpulse = shouldReduceMotion
      ? 0
      : S_peak.value *
        Math.exp(-t / tau_s) *
        Math.sin(2 * Math.PI * f_slosh * t);
    // Device roll to slope
    const roll = rotation.sensor.value ? rotation.sensor.value.roll : 0;
    const tiltSlope =
      enableTilt && !shouldReduceMotion ? clamp(roll * 0.08, -0.25, 0.25) : 0;
  ...
  const backWavePath = useDerivedValue(() => {
    ...
    const A_idle =
      scaledIdleAmplitude *
      0.7 *
      (shouldReduceMotion ? 1 : 1 + 0.2 * breathPhase);
    const t = splashT.value * (scaledSplashDurationMs / 1000);
    const tau = scaledSplashDurationMs / 1000 / 2.2;
    const f_splash = 2.2;
    const A_impulse =
      ...
      0.6 *
      Math.exp(-t / tau) *
      Math.cos(2 * Math.PI * f_splash * t + 0.3);
    const A_total = Math.max(0, A_idle + A_impulse);
    const tau_s = 0.8;
    const f_slosh = 1.4;
    const slopeImpulse = shouldReduceMotion
      ...
      : S_peak.value *
        0.7 *
        Math.exp(-t / tau_s) *
        Math.sin(2 * Math.PI * f_slosh * t + 0.2);
    const roll = rotation.sensor.value ? rotation.sensor.value.roll : 0;
    const tiltSlope =
      enableTilt && !shouldReduceMotion ? clamp(roll * 0.08, -0.25, 0.25) : 0;
    const slopeTotal = clamp(slopeImpulse + tiltSlope, -0.25, 0.25);
  ...
  // Animated props for ring gradient transition
  const normalRingAnimatedProps = useAnimatedProps(() => ({
    strokeOpacity: 1 - goalAchievedOpacity.value,
  }));

  const goalRingAnimatedProps = useAnimatedProps(() => ({
    strokeOpacity: goalAchievedOpacity.value,
  }));

  // Animated bubble props factory (each bubble is a small circle)
  const AnimatedCircle = Animated.createAnimatedComponent(Circle);
  const makeBubbleProps = (seed: {
    id: number;
    x: number;
    y: number;
    r: number;
    ...
    useAnimatedProps(() => {
      ...
      const t =
        (0.5 + (bubblesTime.value * seed.speed + seed.phase) / (2 * Math.PI)) %
        1;
      const y = bottom - t * travel;
      // only show when under the surface
      const isVisible = y > waterLevel + 3;
      const drift =
        Math.sin(phase.value * 0.8 + seed.phase) *
        (4 * seed.drift * (width / 240)) *
        bubbleDriftScale;
  ...
  const bubbleDriftScale =
    perf === 'high' ? 1 : perf === 'balanced' ? 0.85 : 0.65;
  ...
  const bubbleCount = Math.max(
    3,
    Math.min(8, Math.round(6 * scaleFactor * bubblesMultiplier)),
  );
  ...
  // Accessibility label
  const accessibilityLabel = `Water level ${valueMl} milliliters, ${Math.round(
    progress * 100,
  )} percent of goal ${goalMl} milliliters`;
      ...
      <Svg width={size} height={size} style={styles.svg}>
        <Defs>
          ...
          <LinearGradient id="waterGrad" x1="0" y1="0" x2="0" y2="1">
            <Stop
              offset="0%"
              stopColor={colors.waterGradient[0]}
              stopOpacity={1}
            />
            <Stop
              offset="100%"
              stopColor={colors.waterGradient[1]}
              stopOpacity={1}
            />
          </LinearGradient>
          ...
          {/* Goal achieved gradient - light green */}
          <LinearGradient id="goalAchievedGrad" x1="0" y1="0" x2="1" y2="1">
            <Stop offset="0%" stopColor="#86EFAC" stopOpacity={0.9} />
            <Stop offset="25%" stopColor="#4ADE80" stopOpacity={0.85} />
        ...
        {/* Ring on top to remain visible above water - normal rim */}
        <AnimatedCircle
          cx={center}
          cy={center}
          r={radius}
          ...
          fill="transparent"
          animatedProps={normalRingAnimatedProps}
        />
        {/* Ring overlay - goal achieved green gradient */}
        <AnimatedCircle
          cx={center}
          cy={center}
          r={radius}
          stroke="url(#goalAchievedGrad)"
          strokeWidth={strokeWidth}
          fill="transparent"
          animatedProps={goalRingAnimatedProps}
        />
