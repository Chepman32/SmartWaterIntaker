...
const formatGapDuration = (hours: number) => {
...
const AIScreen: React.FC = () => {
  ...
  const { summary, notifications } = useMemo(
    ...
    [events, dailyGoalMl, todayTotalMl, profile, reminderSnapshot]
  );
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={[styles.heroCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
          ...
          <View style={[styles.progressWrapper, { backgroundColor: theme.progressTrack }]}>
            <View
              style={[
                styles.progressFill,
                {
  ...
  return (
    <View style={[styles.notificationCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
      <View style={styles.notificationHeader}>
        <Text style={[styles.notificationIcon, { color: accent }]}>{item.icon}</Text>
        <View style={styles.notificationHeaderText}>
          <Text style={[styles.notificationTitle, { color: theme.text }]}>{item.title}</Text>
          <Text style={[styles.notificationTone, { color: accent }]}>{item.tone}</Text>
        </View>
      </View>
      <Text style={[styles.notificationBody, { color: theme.textSecondary }]}>{item.body}</Text>
      ...
      {item.recommendations?.length ? (
        <View style={styles.recommendations}>
          {item.recommendations.map((rec, index) => (
            <View key={`${item.id}-rec-${index}`} style={styles.recommendationRow}>
      ...
      {item.tags?.length ? (
        <View style={styles.tagRow}>
          {item.tags.map((tag) => (
            <View key={`${item.id}-${tag}`} style={[styles.tagChip, { backgroundColor: `${accent}25` }]}>
